...
export function calculateGST(amount, category) {
    ...
    if (!(category in categories) || category.length == 0) return null;

    const gstRate = categories[category];
    ...
    // return `${amount}, ${categories["category"]}, ${gstAmount}, #{parseFloat(totalAmount.toFixed(2))} `;
}
